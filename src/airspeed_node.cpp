...
int main (int argc, char** argv){
    ...
    unsigned char xml_parser_on[] = { 0x7E, 0x00, 0x10, 0x17, 0x01, 0x00, 0x13, 0xA2, 0x00, 0x40, 0x30, 0xEF, 0xE6, 0xFF, 0xFE, 0x02, 0x44, 0x31, 0x05, 0x74 };
    std::vector<unsigned char> airspeed_on(xml_parser_on, xml_parser_on+20);

    unsigned char xml_parser_off[] = { 0x7E, 0x00, 0x10, 0x17, 0x01, 0x00, 0x13, 0xA2, 0x00, 0x40, 0x30, 0xEF, 0xE6, 0xFF, 0xFE, 0x02, 0x44, 0x31, 0x04, 0x75 };
    std::vector<unsigned char> airspeed_off(xml_parser_off, xml_parser_off+20);
    ...
    while(ros::ok()){
        ...
        if(ser.available()){
            ...
            //    std::cout << response[i] << ' ';
            float x = ((response[25])*256 + (response[26])*1)*3.2;
